...
export default function DoseList() {
  ...
  const takenMornnig = doses?.morning?.filter(
    (dose) => dose.status === "taken",
  );
  const takenNoon = doses?.noon?.filter((dose) => dose.status === "taken");
  const takenEvenning = doses?.evening?.filter(
    (dose) => dose.status === "taken",
  );

  const taken =
    (takenEvenning?.length || 0) +
    (takenMornnig?.length || 0) +
    (takenNoon?.length || 0);

  const totalDose =
    doses?.morning?.length + doses?.evening?.length + doses?.noon?.length;
    ...
    <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
      <div className="flex justify-between items-center">
        ...
        <button className="text-blue-600 text-sm font-medium cursor-pointer">
          <Link href="/dashboard/patient/pilulier">Voir tout</Link>
        </button>
      </div>
            ...
            >
              <div>
                ...
                <p className="text-xs text-gray-500">
                  Morning ·{" "}
                  {dose.medicine.dosage.amount +
                    " " +
                    dose.medicine.dosage.unit}
                </p>
              </div>
